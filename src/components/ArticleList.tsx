...
import { useGetAnimesQuery, useGetIdsQuery, useGetSearchIdsQuery } from '../store/API/anime';
    ...
    const {
        ...
        !isSearching && successIds && ids.length !== 0
            ...
            : isSearching && successSearchIds && searchIds.length !== 0
                ? searchIds.slice(page * perPage, (page + 1) * perPage)
                : skipToken
    );

    const loading =
        loadingIds || loadingAnimes || fetchingIds || fetchingAnimes || loadingSearchIds || fetchingSearchIds;
