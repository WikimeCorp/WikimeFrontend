...
const Navbar = () => {
    ...
    const imgUrl = user && avatar && avatar.includes('images') ? `http://${apiHost}${auth.user?.avatar}` : avatar;
    ...
    return (
        <div className="navbar">
            <header className="content">
                ...
                <div className="links">
                    <NavLink to="/articles" className={({ isActive }) => (isActive ? 'active' : 'not-active')}>
                        Статьи
                    </NavLink>
                    {user && user.role !== 'user' && (
                        <NavLink to="/add" className={({ isActive }) => (isActive ? 'active' : 'not-active')}>
                            Добавить
                        </NavLink>
                    )}
                    {user && user.role !== 'user' && user.role !== 'moderator' && (
                        <NavLink to="/admin" className={({ isActive }) => (isActive ? 'active' : 'not-active')}>
                            Админ
                        </NavLink>
                    )}
                </div>
